...
%         all_trial_info = load('W:\Connie\results\Bassi2025\fig3\sound_info\active_all_trial_info.mat').all_trial_info; %all_trial_info
        ...
        rng(5);
        bframes2 = []; bframes_sounds=[];
        % Define the frames for after and before stimulus
        if ~isempty(before_after)
            aframes =before_after{2};  % After stimulus
            bframes = before_after{1};   % Before stimulus
            if length(before_after)>2
                 bframes_sounds = before_after{3};
            end
   ...
   for split = 1:splits
       split
        ...
        for current_dataset = chosen_mice
            ...
            for celltype = chosen_celltypes
                ...
                stim_matrix = [dff_st{1,current_dataset}.stim(:,mod_cells,:); dff_st{2,current_dataset}.stim(:,mod_cells,:)];
                ctrl_matrix = [dff_st{1,current_dataset}.ctrl(:,mod_cells,:); dff_st{2,current_dataset}.ctrl(:,mod_cells,:)];
                if isfield(run_params,'use_behavior_regression') && run_params.use_behavior_regression
                
                    behavior_stim_matrix = build_behavior_matrix_for_axis( ...
                        run_params.behavior, ...
                        run_params.behavior_vars, ...
                        current_dataset, ...
                        'stim');
                
                    behavior_ctrl_matrix = build_behavior_matrix_for_axis( ...
                        run_params.behavior, ...
                        run_params.behavior_vars, ...
                        current_dataset, ...
                        'ctrl');
                 ...
                 if run_params.avg_frames == 1
                    stim_matrix = regress_out_behavior_3d( ...
                        stim_matrix, behavior_stim_matrix, trainA_stim_all,bframes);
                
                    ctrl_matrix = regress_out_behavior_3d( ...
                        ctrl_matrix, behavior_ctrl_matrix, trainA_ctrl_all,bframes);
                 else %regresses each frame!!
                     stim_matrix = regress_out_behavior_3d( ...
                        stim_matrix, behavior_stim_matrix, trainA_stim_all);
                
                    ctrl_matrix = regress_out_behavior_3d( ...
                        ctrl_matrix, behavior_ctrl_matrix, trainA_ctrl_all);
                 end
                end
    ...
    %             [trials_ctrl] = size(ctrl_matrix, 1);
                ...
                for tr = trainA_ctrl_all;%train_trials_ctr %1:trials_ctrl
                    ...
                    if ~isempty(bframes_sounds)
                        prepost_trial(tr,:,:) = [nanmean(data_matrix(tr, :, aframes), [3]);
                                           nanmean(data_matrix(tr, :, bframes_sounds), [3])]; % [post, pre]
                    end
                end
...
function behavior_matrix = build_behavior_matrix_for_axis(behavior, behavior_vars, dataset, trial_type)
% behavior.speed{dataset,context}.ctrl or .stim
% behavior.pitch{dataset,context}.ctrl or .stim
%
% Output:
% behavior_matrix = trials x predictors x frames
% contexts concatenated as active then passive, matching:
% [dff_st{1,dataset}.stim; dff_st{2,dataset}.stim]

    n_vars = numel(behavior_vars);
    behavior_by_var = cell(1,n_vars);

    for v = 1:n_vars
        varname = behavior_vars{v};

        active_data  = behavior.(varname){dataset,1}.(trial_type);
        passive_data = behavior.(varname){dataset,2}.(trial_type);

        % Ensure trials x 1 x frames
        active_data  = force_trials_x_1_x_frames(active_data);
        passive_data = force_trials_x_1_x_frames(passive_data);

        behavior_by_var{v} = cat(1, active_data, passive_data);
    end

    behavior_matrix = cat(2, behavior_by_var{:});
end

function x = force_trials_x_1_x_frames(x)

    if ndims(x) == 2
        % assume trials x frames
        x = reshape(x, size(x,1), 1, size(x,2));
    elseif ndims(x) == 3
        % already okay if trials x 1 x frames
        if size(x,2) ~= 1
            error('Behavior variable should be trials x 1 x frames or trials x frames.');
        end
    else
        error('Behavior variable has unexpected dimensions.');
    end
end